...
class OrmWrapper():
    ...
    def load_class(netloc, module_name, class_name):
        ...
        try:
            ...
            logger.warning("OrmWrapper.load_class not found netloc %s, module_name %s, class_name %s: _%s" % (
            netloc, module_name, class_name, str(ex)))
            raise (ex)
    ...
    def load_classes(module_name, class_name):
        ...
        try:
            classes = []
            mms = ks.models.ModelMetadata.filter(module=module_name, name=class_name)
            for mm in mms:
                classes.append(OrmWrapper.load_class(ks.utils.KsUrl(mm.UKCL).netloc, module_name, class_name))

            return classes
        except Exception as ex:
            logger.warning("OrmWrapper.load_classes not found module_name %s, class_name %s: _%s" % (
            module_name, class_name, str(ex)))
            raise (ex)
    ...
    def get_model_container_name(netloc, name):
        ...
        try:
            if netloc == "root.beta.thekoa.org":
                return name
            if name in ks.models.KnowledgeServer.root_apps:
                return name
            this_ks = ks.models.KnowledgeServer.this_knowledge_server()
            that = netloc.split('.')
    ...
    def get_model_metadata_netloc(app_label):
        ...
        # even in other OKSs
        if app_label in ks.models.KnowledgeServer.root_apps:
    ...
    def load_dynamic_apps():
        dmcs = ks.models.DynamicModelContainer.objects.all()
        for dmc in dmcs:
