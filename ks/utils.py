...
class KsUrl(object):
    ...
    def __init__(self, url):
        ...
        '   così possiamo rimuovere il fragment e params query in modo da ripulire l'url ed essere più forgiving sulle 
        '   api; da valutare
        '''
        self.url = url.strip()
        self.parsed = urlparse(self.url)
        # I remove format options if any, e.g.
        # http://root.thekoa.org/knowledge_server/ModelMetadata/1/json/
        # -->
        # http://root.thekoa.org/knowledge_server/ModelMetadata/1/
        self.clean_uri = url
        ...
        self.format = ""
        for f in Choices.FORMAT:
            if self.clean_uri[-(len(f) + 1):].lower() == "/" + f:
                self.clean_uri = self.clean_uri[:-(len(f) + 1)]
                self.format = f
        ...
        if self.scheme and self.netloc and self.path:
            ...
            # where "knowledge_server" is the module/app/namespace, "ModelMetadata" is the class name
            # and "1" is the id or pk
            temp_path = self.path
            if temp_path[0] == "/":
    ...
    def encoded(self):
        return urlencode({'': self.url})[1:]

    def home(self):
        return self.scheme + '://' + self.netloc

    def __repr__(self):
        return self.scheme + '://' + self.netloc + '/' + self.namespace + '/' + self.class_name + '/' + str(
            self.pk_value)
    ...
    def search_on_db(self):
        ...
        if self.is_sintactically_correct:
            ...
            try:
                self.knowledge_server = ks.models.KnowledgeServer.objects.using('materialized').get(
                    scheme=self.scheme, netloc=self.netloc)
                self.is_ks_known = True
            except:
                self.is_ks_known = False
            # I search for its class
            try:
                self.actual_class = ks.orm_wrapper.OrmWrapper.load_class(self.netloc, self.namespace,
                                                                         self.class_name)
            except:
                pass
